...
class Editor {
	...
	constructor(editorId: string) {
		this.text = <HTMLTextAreaElement>document.getElementById(editorId);
		$(".btn-editor").on("click",(e) => this.onClick(e));
	}
	...
	private doTag(tag: string, parameter?: string) {
		var startTag;
		...
		if (document.selection) {
            this.text.focus();
            var selectedRange: TextRange = document.selection.createRange();
	...
	private doList(isOrdered: boolean) {
		var startTag = "[list]";
		if (isOrdered == true)
			startTag = "[list=1]";
		var endTag = "[/list]";
